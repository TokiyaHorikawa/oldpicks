class ArticlesController < ApplicationController
  ...
  def index
    ...
    @business_articles = Article.tagged_with("ビジネス").order("created_at DESC").limit(3)
    @economy_articles = Article.tagged_with("政治").order("created_at DESC").limit(3)
    @finance_articles = Article.tagged_with("金融").order("created_at DESC").limit(3)
    @career_articles = Article.tagged_with("キャリア").order("created_at DESC").limit(3)
    @society_articles = Article.tagged_with("社会").order("created_at DESC").limit(3)
    @job_articles = Article.tagged_with("ジョブ").order("created_at DESC").limit(3)
    @back_number_articles = Article.tagged_with("過去記事").order("created_at DESC").limit(3)
    @analysis_articles = Article.tagged_with("アナリスト").order("created_at DESC").limit(3)
